    ...
    return (
        <div>
            <div className="flex justify-evenly my-6 mb-10">
                <h2 className="text-xl md:text-4xl lg:text-4xl font-bold">All <span className='text-blue-600'>Customer List</span></h2>
             ...
             <div className=" ml-10">
                 <table className="table  border-separate w-3/4 md:w-1/2 lg:w-full">
                    ...
                    <tbody>
                        {/* row 1 */}
                        {
                            users.map((user,index) => <tr key={user._id}  className={index % 2 === 0 ? 'bg-slate-200' : 'bg-blue-200'}>
                                ...
                                <th>
                                   {
                                    user.role === 'admin' ? <button onClick={() => handleRemoveAdmin(user)} className='btn bg-blue-600 btn-xs text-white font-bold'> Remove Admin</button> : 
                                    <button 
            ...
            <div className="text-center  mt-10 mb-10">
                <Button colorScheme='blue' variant="outline" onClick={handlePreviousPage} isDisabled={page === 0} className="btn mr-1 btn-sm bg-blue-600 text-white">{<ArrowLeftIcon />}</Button>
                {pages.map((pageNumber, index) => {
                    if (index === 0 || index === totalPages - 1 || (index >= page - Math.floor(pagesToShow / 2) && index <= page + Math.floor(pagesToShow / 2))) {
                        return (
                            <Button fontWeight='bold'  isDisabled={page === pageNumber} key={index} onClick={() => setPage(pageNumber)} className={`btn btn-sm mr-1 border ${page === pageNumber ? "bg-slate-300 text-black" : "bg-blue-600 text-white"}`}>{pageNumber + 1}</Button>
                        );
                ...
                <Button colorScheme='blue' variant="outline" onClick={handleNextPage} isDisabled={page === totalPages - 1} className="btn ml-1 btn-sm bg-blue-600 text-white">{<ArrowRightIcon />}</Button>
            </div>
        </div>
    );
};
